...
from .wrappers import report_uncaught, threaded
...
class ClassifierTool(tk.Frame):
    ...
    def __init__(
        self, parent, standard_program, classifier, utilities, *args, **kwargs
    ):
    ...
    def check_enable_predictions(self):
        ...
        """
        conditions = [self.sp.data_file is not None, self.sp.clf.is_trained()]
        # print(conditions)
        if all(conditions):
    ...
    def get_data_file(self, internal=None):
        ...
        def _get_data_file(self, data_file, on_finish=None):
            ...
            if (
                os.path.splitext(data_file)[1]  # some sort of excel file
                ...
                except (OSError, KeyError, TypeError, ValueError, XLRDError):
                ...
                self.data_sec.excel_sheet_input.config(state=tk.DISABLED)

                # in case there's a read or parse error of some kind
                try:
                    column_names = self.util.get_data(data_file).columns.values.tolist()
                ...
                self.data_sec.col_select_panel.update_contents(colnames_dict)
        ...
        _get_data_file(
            self, data_file, on_finish=lambda: finish(self, progress.complete)
        )
    ...
    def get_selected_cols(self):
        ...
        """
        # get selection
        selected_cols = list(self.data_sec.col_select_panel.sel_contents.get(0, tk.END))
    ...
    def get_sheet_cols(self, event):
        ...
        def _get_sheet_cols(self, sheet, on_finish=None):
            try:
                ...
                self.dlib.dialog(messagebox.showwarning, "FILE_READ_ERR_AFTER_SUCCESS")
        ...
        progress = ProgressPopup(self, "Reading Excel File", "Reading sheet...")
        ...
        _get_sheet_cols(self, sheet, on_finish=lambda: finish(self, progress.complete))
    ...
    def load_classifier(self):
        ...
        def _load_classifier(self, clf_file, on_finish=None):
            ...
            try:
                ...
                self.dlib.dialog(messagebox.showwarning, "INCOMPATIBLE_NOT_CLF")
                on_finish()
                return
            except OSError:
        ...
        clf_file = filedialog.askopenfilename(
            ...
            filetypes=[("Classifier archive file", ".gz .joblib .pickle")],
        )
        ...
        progress = ProgressPopup(self, "Loading Classifier", "Unpacking file...")
        ...
        _load_classifier(
            self, clf_file, on_finish=lambda: finish(self, progress.complete)
        )
    ...
    def make_cm(self, features_known, class_labels):
        ...
        self.util.make_confm(self.sp.clf.clf, features_known, class_labels).savefig(
            self.cm.name
        )
    ...
    def make_pairplot(self, features, predict):
        ...
        if len(features.columns) > 25:
            if len(features.columns) > 92:
                self.dlib.dialog(messagebox.showwarning, "NO_PAIRPLOT_OVERLOAD")
        ...
        self.util.make_pairplot(features, predict).savefig(self.pairplot.name)
    ...
    def make_predictions(self):
        ...
        def _make_predictions(self, status_cb, on_finish=None):
            ...
            self.dlib.dialog(messagebox.showinfo, "PREDICT_COMPLETE")
        ...
        if len(self.data_sec.col_select_panel.sel_contents.get(0, tk.END)) <= 0:
            self.dlib.dialog(messagebox.showwarning, "NO_COLUMNS_SELECTED")
            return

        # check if user is overwriting and make sure they're ok with it
        if self.predict_sec.output_save["state"] != tk.DISABLED:
            if not self.dlib.dialog(messagebox.askokcancel, "PREDICTION_OVERWRITE"):
                return
        ...
        progress_popup = ProgressPopup(self, "Making Predictions", "")
    ...
    def open_data_tool(self):
        ...
        # open new data tool
        self.data_win = DataPreparationTool(self, self, self.tempdir, self.util)
    ...
    def save_classifier(self):
        ...
        location = tk.filedialog.asksaveasfilename(
            ...
            initialdir=os.path.realpath(os.path.join(self.util.USER_PATH, "output/")),
            defaultextension=".gz",
    ...
    def save_item(self, item):
        ...
        tempfiles = {
            ...
            "output": self.output,
            "bold_results": (
                self.data_win.sequence_raw if self.data_win is not None else None
            ),
            "merged_results": (
                self.data_win.merged_raw if self.data_win is not None else None
            ),
            "filtered_data": (
                self.data_win.sequence_filtered if self.data_win is not None else None
            ),
            "raw_fasta": (self.data_win.fasta if self.data_win is not None else None),
            "fasta_align": (
                self.data_win.fasta_align if self.data_win is not None else None
            ),
            "finalized": (
                self.data_win.finalized_data if self.data_win is not None else None
            ),
            "delimitation": (
                self.data_win.delim if self.data_win is not None else None
            ),
        }
            ...
            if (in_ext != out_ext) and (out_ext in (".csv", ".tsv")):
                try:
                    self.util.get_data(tempfiles[item].name).to_csv(
                        location, sep=("\t" if out_ext == ".tsv" else ","), index=False
                    )
                except OSError:
        ...
        self.status_bar.set_status(f"Saving {item}...")
    ...
    def save_nans(self):
        ...
        def _save_nans(self, on_finish=None):
            ...
            try:
                with open(
                    os.path.join(self.util.USER_PATH, "config/nans.json"), "w"
                ) as nansfile:
    ...
    def train_classifier(self):
        ...
        def _train_classifier(self, status_cb, on_finish=None):
            ...
            if removed_rows > 0:
                self.dlib.dialog(
                    messagebox.showwarning, "REMOVED_ROWS_NO_DATA", form=(removed_rows,)
                )
            ...
            if not self.sp.check.check_enough_classes(
                ...
                lambda: self.dlib.dialog(
                    messagebox.showerror, "NOT_ENOUGH_CLASSES_TO_TRAIN"
                ),
            ):
                on_finish()
                return
            ...
            if not self.sp.check.check_minimum_training_labels(
                metadata,
                lambda: self.dlib.dialog(
                    messagebox.showerror, "NOT_ENOUGH_KNOWN_SAMPLES"
                ),
            ):
                on_finish()
                return
            ...
            if not self.sp.check.check_inbalance(
                metadata,
                lambda: self.dlib.dialog(messagebox.askokcancel, "HIGH_IMBALANCE"),
            ):
                on_finish()
                return
            ...
            self.dlib.dialog(messagebox.showinfo, "TRAIN_COMPLETE")
        ...
        if self.train_sec.known_select.get() in self.get_selected_cols():
            self.dlib.dialog(messagebox.showwarning, "ERR_OVERLAP_FEATURES_CLASS")
            return
        ...
        progress_popup = ProgressPopup(self, "Training Classifier", "")

        # start threaded portion
        _train_classifier(
            self, progress_popup.set_status, on_finish=progress_popup.complete
        )
    ...
    def view_item(self, item):
        ...
        try:
            ...
            if self.dlib.dialog(
                ...
                subprocess.Popen(f"explorer /select,{item}")
