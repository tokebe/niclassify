...
class NaNEditor(tk.Toplevel):
    ...
    def __init__(self, parent, app, *args, **kwargs):
        ...
        self.items = tk.Listbox(
            self.itemframe, selectmode=tk.EXTENDED, listvariable=self.item_var
        )
        self.items.pack(side=tk.LEFT, expand=True, fill=tk.BOTH)

        self.item_scroll = tk.Scrollbar(self.itemframe, orient=tk.VERTICAL)
        ...
        self.addbutton = tk.Button(self.buttonframe, text="Add", command=self.additem)
        self.addbutton.pack(fill=tk.X)

        self.removebutton = tk.Button(
            self.buttonframe, text="Remove", command=self.removeitem
        )
    ...
    def removeitem(self):
        ...
        items = [
            ...
            for i, j in enumerate(
                list(ast.literal_eval(self.item_var.get()))
...
class ProgressPopup(tk.Toplevel):
    ...
    def __init__(self, parent, title, status, *args, **kwargs):
        ...
        self.align = tk.Frame(self, padx=10, pady=10)
        self.align.pack()

        # status label
        self.status = tk.Label(self.align, text=status, pady=5)
        self.status.pack(anchor=tk.W)

        # progress bar
        self.progress = ttk.Progressbar(
            self.align, orient=tk.HORIZONTAL, mode="indeterminate", length=250
        )
        self.progress.pack()
